...
public class JwtUtils {
    ...
    @Value("${mentara.app.jwtSecret}")
    private String jwtSecret;

    @Value("${mentara.app.jwtExpirationMs}")
    private int jwtExpirationMs;

    @Value("${mentara.app.jwtRefreshExpirationMs}")
    ...
    public boolean validateRefreshToken(String refreshToken) {
        try {
            Jwts.parserBuilder().setSigningKey(key()).build().parse(refreshToken);
            return true;
        } catch (MalformedJwtException e) {
    ...
    public String getUserNameFromRefreshToken(String refreshToken) {
        return getUserNameFromJwtToken(refreshToken);
    }
}